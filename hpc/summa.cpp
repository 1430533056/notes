...
void gatherMatrix(int mb, int nb, float *loc, int bM, int bN, float *global,
                  MPI_Comm comm) {
  float *A_tmp = NULL;
  if (rank == 0) {
...
void scatterMatrix(int m_block, int n_block, int bM, int bN, float *local,
                   float *global, MPI_Comm comm, int m, int n, int N,
                   int id = 0) {
  float *temp = nullptr;
  if (rank == 0 || rank == id) {
    // printf("rank:%d\n", rank);
  ...
  MPI_Scatter(temp, m_block * n_block, MPI_FLOAT, local, m_block * n_block,
              MPI_FLOAT, 0, comm);
  if (rank == 0 || rank == id) {
    delete[] temp;
  }
...
void matMul(int m_block, int k_block, int n_block, float *A_local,
            float *B_local, float *temp_c) {
...
int compute(float *C, float *A, float *B, float *C_recv) {
  ...
  MPI_Comm groups1;
  MPI_Comm_split(comm_cart, int(my_row < k_split && my_col < k_split), rank,
                 &groups1);
  ...
  for (int k = 0; k < K; k += bK) {
    ...
    scatterMatrix(m_block, k_block, bM, bK, A_local, A, groups1, 0, k, K,
                  other_id);
    scatterMatrix(k_block, n_block, bK, bN, B_local, B, groups2, k, 0, N,
                  other_id);
