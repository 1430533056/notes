...
template <typename T> struct Expr {
  const T &self(void) const { return *(static_cast<const T *>(this)); }
};
...
struct Vec : public Expr<Vec> {
  ...
  float eval(int i) const { return data[i]; }
  template <typename T> Vec &operator=(const Expr<T> &p) {
    for (int i = 0; i < len; i++) {
...
struct BinaryOp : public Expr<BinaryOp<LH, RH, Op>> {
  ...
  float eval(int i) const { return Op::Map(lh.eval(i), rh.eval(i)); }
};
